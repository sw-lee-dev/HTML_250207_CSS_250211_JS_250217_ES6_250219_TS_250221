...
/*
    sort(콜백 함수) : 배열의 요소를 정렬(고차 함수는 아님)
    - .sort() 로 정렬하면 사전식 정렬(사전식 오름차순)이 이루어짐
*/
array.sort();
console.log(array);
...
/*
    ...
    - 원본 배열 변경 X
    - break, continue 등 일반 반복문에서 사용되는 부수 작업을 수행할 수 없음
    - for 문 보다 성능이 많이 떨어짐(2배 이상 느림), 가독성이 좋고 편리해서 사용
    - async await 작업(동기 작업)은 forEach 메서드로 사용 불가능
*/
...
/*
    ...
    - 원본 배열 변경 X
    - forEach는 단순히 요소나 인덱스를 가지고 특정 작업만 수행 (return X)
    - map은 요소나 인덱스를 가지고 작업한 결과물을 반환 (return 필수)
*/
...
/*
    find(콜백 함수) : 콜백 함수의 결과가 true인 첫번째 요소를 반환 (존재하지 않으면 undefined 반환)
*/
...
/*
    findIndex(콜백 함수) : 콜백 함수의 결과가 true인 첫번째 요소의 인덱스를 반환 (존재하지 않으면 -1 반환)
*/
